...
> [!cite] 教材定位
> 原书：[[408/90-复习资料/01-核心教材/2026数据结构_带书签.pdf#page=24|第2章 线性表（PDF 第 24 页）]]；本章范围为 PDF 第 24–73 页。
...
设计 $O(n)$ 时间、$O(1)$ 额外空间算法，稳定删除顺序表中所有值为 $x$ 的元素。以 `[2,1,2,3,2]`、$x=2$ 为例说明过程。

> [!success]- 展开完整答案
> 用写指针 `k` 表示已经保留的元素个数。读指针从左到右扫描，遇到不等于 $x$ 的元素就写入 `a[k]` 并令 `k++`：
>
> ```c
...
> ```
>
> 扫描示例：2 跳过；1 写到位置 0；2 跳过；3 写到位置 1；2 跳过。有效区间变为
>
> ```text
> [1, 3]
> ```
>
> 每个元素读一次，时间 $\Theta(n)$；只用常数指针，额外空间 $O(1)$。保留元素的相对次序没有改变，所以算法稳定。

### 例 2：单链表倒数第 k 个结点

只允许遍历一次单链表，如何找到倒数第 $k$ 个结点？说明 $k$ 越界的判断。

> [!success]- 展开完整答案
> 设 `fast`、`slow` 都指向首元结点。先让 `fast` 前进 $k$ 步：
>
> - 若前进过程中提前遇到 `NULL`，说明链长小于 $k$，无答案；
> - 否则同时移动两指针，直到 `fast==NULL`，此时 `slow` 正好指向倒数第 $k$ 个结点。
>
> ```c
...
> ```
>
> 两指针始终相距 $k$ 个结点。时间 $O(n)$、空间 $O(1)$。若链表带头结点，应传入首元结点或明确把头结点排除在计数外。

## 做题识别顺序

1. 先明确顺序表下标和链表是否带头结点，写出空表与合法区间。
2. 顺序表插入从后向前移，删除从前向后覆盖，避免覆盖尚未搬走的数据。
3. 链表改指针前先画前驱—当前—后继三个结点，并保存即将丢失的地址。
4. 综合题优先识别双指针、快慢指针、哨兵、原地逆置和摘链合并。
5. 复杂度分成“找到位置”和“执行操作”两部分，不把已知结点位置与按序号定位混为一谈。

## 一页记忆

- 顺序表随机访问 $O(1)$，按位置插删通常 $O(n)$；链表按序号定位 $O(n)$，已知局部指针后改链可 $O(1)$。
- 单链表删除结点必须先保存后继；双链表插入/删除要同时维护前向和后向链。
- 快慢指针可求中点、判环；固定间距双指针可求倒数第 $k$ 个。
- 循环链表没有 `NULL` 尾标志；只设尾指针时，首结点可由 `tail->next` 在 $O(1)$ 时间得到。
...
> [!question]- 自测问题参考答案
> 1. 若从前向后移动，先写入的元素会覆盖尚未复制的后继；从后向前可保证每个源元素在被覆盖前先搬走。
> 2. 若只给“第 $i$ 个位置”，先定位前驱仍需 $O(n)$；只有已经得到适当结点/前驱指针时，局部改链才是 $O(1)$。
> 3. 给定结点是尾结点时不能用“复制后继数据再删后继”的技巧；单链表还需要找到其前驱才能真正删除尾结点，通常为 $O(n)$。
> 4. 设写指针 `k=0`，从左到右扫描，把所有不等于 $x$ 的元素依次写到 `a[k++]`，最后把长度改为 `k`。扫描顺序不变，因此稳定。
> 5. 首结点是 `tail->next`，尾结点已有指针，所以在表头和表尾插入都可 $O(1)$；两个循环链表拼接也只需修改少量指针。

## 资料依据

- 《2026 年数据结构考研复习指导》第 2 章，第 24～73 页；按 PDF 书签定位并以定向 OCR 辅助核对，边界和指针顺序已人工复核。
- 本目录既有顺序表、链表算法与真题分类用于交叉核对。
