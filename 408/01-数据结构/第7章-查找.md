...
> [!cite] 教材定位
> 原书：[[408/90-复习资料/01-核心教材/2026数据结构_带书签.pdf#page=281|第7章 查找（PDF 第 281 页）]]；本章范围为 PDF 第 281–348 页。
...
有序表含关键字 1～7，按 `mid=(low+high)/2` 向下取整进行折半查找，各关键字等概率。求成功 ASL。

> [!success]- 展开完整答案
> 第一次比较中点 4；第二层比较 2、6；第三层比较 1、3、5、7。各层结点的比较次数分别为 1、2、3，所以
>
> $$
> ASL_{succ}
> =\frac{1\times1+2\times2+4\times3}{7}
> =\frac{17}{7}
> \approx\boxed{2.43}.
> $$
>
> ASL 是带概率的期望比较次数，不是树高。若关键字概率不等，就要按 $\sum p_i c_i$ 加权；失败 ASL 的样本空间是各个查找失败区间，必须另算。

### 例 2：AVL 的 LR 双旋

向空 AVL 树依次插入 30、10、20，说明失衡类型和调整过程。
...
> ```
>
> 最低失衡祖先是 30。新结点位于 30 的左孩子 10 的右子树，所以是 LR 型：
>
> 1. 先对 10 左旋，使 20 上升；
> 2. 再对 30 右旋，使 20 成为该子树新根。
>
> ```text
>      20
>     /  \
...
> ```
>
> 三个结点的中序次序仍为 10、20、30，高度恢复平衡。判断 LL/LR/RL/RR 应看“从最低失衡祖先到新结点的前两步方向”。

### 例 3：线性探测散列表

散列表长度 11，散列函数 $H(k)=k\bmod11$，线性探测，依次插入 22、1、13、11、24。写出位置和成功 ASL。

> [!success]- 展开完整答案
> - 22：$H=0$，放 0，比较 1 次；
> - 1：$H=1$，放 1，比较 1 次；
> - 13：$H=2$，放 2，比较 1 次；
> - 11：$H=0$，0、1、2 均占用，放 3，比较 4 次；
> - 24：$H=2$，2、3 占用，放 4，比较 3 次。
>
> ```text
> index: 0  1  2  3  4
> value: 22 1 13 11 24
...
>
> $$
> \frac{1+1+1+4+3}{5}=\boxed{2}.
> $$
>
> 删除开放定址元素时不能直接置为空，否则从原散列地址出发的查找会在此提前终止；应使用墓碑标记并在适当时机重散列。

## 做题识别顺序

1. ASL 先写样本空间和概率，再为每个成功关键字或失败区间写比较次数。
2. BST/AVL 按序逐个插入；AVL 找离插入点最近的最低失衡祖先，再判四型。
3. B 树每次操作后立即检查关键字数上下界，上溢分裂中间关键字上升，下溢先借后并。
4. 散列表逐关键字写原地址、实际探测序列和比较次数，探测每一步都要取模。
5. 结构选择题同时比较内存/外存、等值/范围、更新频率和最坏界。

## 一页记忆

| 结构 | 平均/最坏查找 | 主要特点 |
|---|---|---|
| 有序表折半 | $O(\log n)$ | 需要随机访问，插删慢 |
| BST | 平均 $O(\log n)$，最坏 $O(n)$ | 性能取决于树高 |
| AVL | $O(\log n)$ | 高度严格平衡，查询稳定 |
| 红黑树 | $O(\log n)$ | 较松平衡，更新旋转较少 |
| B/B+ 树 | $O(\log_m n)$ 次结点访问 | 高扇出，适合外存 |
| 散列 | 平均 $O(1)$，最坏 $O(n)$ | 等值快，不天然有序 |

- $m$ 阶 B 树最多 $m$ 个孩子、$m-1$ 个关键字；除根外至少 $\lceil m/2\rceil$ 个孩子。
- B+ 树数据/记录指针集中在叶层，叶子按关键字链接，范围扫描高效。
...
> [!question]- 自测问题参考答案
> 1. 折半每轮必须 $O(1)$ 定位中点；单链表找中点需沿指针走 $O(n)$，多轮定位总成本不再是 $O(\log n)$，且链表也不能根据下标直接访问。
> 2. 插入只会使沿途高度增加一次，首次调整后子树高度通常恢复；删除可能让调整后的子树高度继续下降，从而使多个祖先依次失衡，需要向根连续调整。
> 3. 红结点不能相邻，且任一结点到所有 NIL 叶的黑高相同，所以最长根叶路径至多是最短路径的 2 倍，可推出高度不超过 $2\log_2(n+1)$。
> 4. B+ 树所有数据项在叶层且叶结点有序链接，定位起点后可顺序扫叶；内部结点只作索引，扇出更高、树更矮。
> 5. 开放定址查找遇到真正空槽才可断言后面不存在目标。删除后若直接置空，会截断其他关键字的探测链；墓碑表示“此处已删但查找应继续”。

## 资料依据

- 《2026 年数据结构考研复习指导》第 7 章，第 281～348 页；按 PDF 书签定位并以定向 OCR 辅助核对，ASL、平衡树、多路树和散列口径已人工复核。
- 本目录既有结构对比和复杂度总结用于交叉核对。
